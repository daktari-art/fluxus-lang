...
export const ANALYTICS_OPERATORS = {
    ...
    'detect_trend': {
        ...
        description: 'Detect trends in time series data using linear regression',
        type: 'analytics'
    },
    ...
    'analyze_cohorts': {
        ...
        description: 'Perform cohort analysis for user retention',
        type: 'analytics'
    },
    ...
    'detect_anomalies': {
        ...
        description: 'Detect anomalies in data using statistical methods',
        type: 'analytics'
    },
    ...
    'calculate_kpis': {
        ...
        description: 'Calculate key performance indicators from business data',
        type: 'analytics'
    },
...
export const registerWithEngine = (engine) => {
    console.log('📊 Registering Analytics Domain...');
    
    let count = 0;
    for (const [operatorName, operatorDef] of Object.entries(ANALYTICS_OPERATORS)) {
        if (!engine.operators.has(operatorName)) {
            // Create wrapped operator with proper context
            const wrappedOperator = async (input, args, context) => {
                const startTime = Date.now();
                
                try {
                    const result = await operatorDef.implementation(input, args, {
                        ...context,
                        domain: 'analytics',
                    ...
                    throw new Error(`[analytics.${operatorName}] ${error.message}`);
                }
            };
    ...
    console.log(`   ✅ Analytics Domain registered: ${count} operators`);
    return count;
};

export default ANALYTICS_OPERATORS;
