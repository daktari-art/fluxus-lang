...
// 
// Fluxus Language Graph Parser v4.1 - WITH Function Support
...
export class GraphParser {
    constructor() {
        ...
        this.functionKeywords = ['OPERATION', 'FUNC', 'function'];
    }
    ...
    parse(sourceCode) {
        ...
        for (let lineNum = 0; lineNum < cleanedLines.length; lineNum++) {
            ...
            // Handle function declarations (NEW)
            if (this.isFunctionDeclaration(line)) {
                this.parseFunctionDeclaration(line, lineNum + 1, ast);
                continue;
            }
    ...
    /**
     * NEW: Check if line is a function declaration
     */
    isFunctionDeclaration(line) {
        return this.functionKeywords.some(keyword => 
            line.trim().startsWith(keyword + ' ')
        );
    }

    /**
     * NEW: Parse function declarations like:
     * OPERATION name() { body }
     * FUNC name(): body
     * function name() { body }
     */
    parseFunctionDeclaration(line, lineNum, ast) {
        const trimmed = line.trim();
        
        // Try to extract function name and body
        let funcName, funcBody;
        
        // Pattern for OPERATION/function name() { body }
        const braceMatch = trimmed.match(/(OPERATION|function)\s+(\w+)\s*\([^)]*\)\s*\{([^}]*)\}/);
        if (braceMatch) {
            funcName = braceMatch[2];
            funcBody = braceMatch[3].trim();
        }
        
        // Pattern for FUNC name(): body  
        const colonMatch = trimmed.match(/FUNC\s+(\w+)\s*\([^)]*\)\s*:\s*(.*)/);
        if (colonMatch) {
            funcName = colonMatch[1];
            funcBody = colonMatch[2].trim();
        }
    ...
    parseStreamPipeline(line, lineNum, ast) {
        ...
        for (let i = 0; i < tokens.length; i++) {
            ...
            if (newNode) {
                ...
                if (i === 0) {
                    // First token is the stream source
                    const streamNode = {
                        id: newNode.id,
                        type: line.startsWith('~?') ? 'STREAM_SOURCE_LIVE' : 'STREAM_SOURCE_FINITE',
                        name: line.startsWith('~?') ? 'STREAM_SOURCE_LIVE' : 'STREAM_SOURCE_FINITE',
                        value: newNode.value,
                        args: [],
                        line: lineNum
                    };
    ...
    determineNodeType(value) {
        ...
        if (value.endsWith(' ->')) return 'POOL_READ';
