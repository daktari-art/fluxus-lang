...
// 
// Fluxus Language Runtime Engine v4.1 - Enhanced with Custom Operations
...
export class RuntimeEngine {
        ...
        for (const poolName in this.ast.pools) {
            ...
            try {
                this.pools[poolName] = {
                    ...
                    history: [this.parseLiteralValue(poolDef.initial)],
                    _updates: 0
                };
            } catch (error) {
    ...
    processNode(node, inputData) {
        ...
        // NEW: Handle custom operations
        if (this.ast.functions && this.ast.functions[node.name]) {
            return this.executeCustomOperation(node.name, inputData, node.args);
        }
    ...
    executeCustomOperation(funcName, inputData, args) {
        ...
        try {
            // Simple JavaScript function execution for custom operations
            // This is a basic implementation - in production you'd want a safer approach
            const funcBody = funcDef.body;
            
            // Replace .value with the actual input data
            ...
            const customFunc = new Function('inputData', `return ${processedBody}`);
            const result = customFunc(inputData);
            ...
            return inputData; // Fallback to passing through
        }
    }
